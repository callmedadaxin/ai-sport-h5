  ...
  <div class="page-wrap home">
    ...
    <header class="header">
      <h1 class="header-title">
        ...
        <img class="logo" src="../assets/image/home-logo2.png" alt="" />
        2026皖美运动汇
      </h1>
      <div class="header-actions">
        <button type="button" class="works-btn" aria-label="作品中心" @click="goWorks">
          <svg
            class="svg-icon svg-icon--12"
    ...
    <section ref="templatesRef" class="templates">
      <div class="section-header">
        ...
        <div class="template-type-filters">
          <button
            type="button"
            class="filter-btn"
            :class="{ active: templateType === 'video' }"
            @click="templateType = 'video'"
          >
            视频换脸
          </button>
          <button
            type="button"
            class="filter-btn"
            :class="{ active: templateType === 'image' }"
            @click="templateType = 'image'"
          >
            图片换脸
          </button>
        </div>
      ...
      <div class="template-grid">
        <div v-if="templatesLoading" class="list-loading">
...
const templateType = ref('video') // 'video' | 'image'

function fetchTemplates() {
  templatesLoading.value = true
  templateApi
    .list({ type: templateType.value })
    .then(res => {
...
watch(templateType, fetchTemplates)
...
function goTemplate(id) {
  const query = { type: templateType.value }
  if (!userStore.isLoggedIn) {
    openLogin(() => router.push({ path: '/template/' + id, query }))
    return
  }
  router.push({ path: '/template/' + id, query })
}
</script>
...
.works-btn {
  ...
  cursor: pointer;
  color: #fff;
  /* backdrop-filter 降级：Android 设备不支持时使用更不透明的背景 */
...
/* 支持 backdrop-filter 的浏览器使用更透明的背景 */
...
.template-bg {
  ...
  width: 100%;
  height: 100%;
  background: linear-gradient(180deg, rgba(255, 255, 255, 0.4) 0%, #fff 114.09%);
  opacity: 0.4;
}
.section-title {
  display: flex;
...
.template-info {
  /* backdrop-filter 降级：已有 background 作为降级 */
  background: linear-gradient(180deg, rgba(58, 58, 58, 0.6) 0%, rgba(0, 0, 0, 0.6) 100%);
...
.template-grid {
  /* Grid 降级方案：先使用 flexbox */
  display: flex;
  flex-wrap: wrap;
  margin: -0.075rem;
  /* 预留至少一行卡片高度，切换 loading 时避免页面跳动 */
  min-height: 3.2rem;
}
.template-grid > * {
...
.list-loading {
  ...
  width: 100%;
  margin: 0.075rem;
  /* 占满 min-height，避免 loading 时高度塌陷 */
  min-height: 3.2rem;
...
.tag.green {
  background: linear-gradient(90deg, #9ecd46 0%, #55612b 100%);
}
.tag.blue {
  background: linear-gradient(270deg, #279dc4 0%, #234aab 100%);
}
.tag.yellow {
  background: linear-gradient(90deg, #ddcd8f 0%, #cb934f 100%);
}
.template-title {
  font-size: var(--font-size-sm);
  color: var(--color-text-inverse);
